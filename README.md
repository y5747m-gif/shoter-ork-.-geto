...
### 📱 اللعب بالوضع الأفقي (Landscape)
اللعبة مصمَّمة لتُلعب **بالعرض** مثل بابجي وفري فاير — ساحة رؤية أوسع وأزرار في متناول الإبهامين:

- **كشف تلقائي للاتجاه**: إن كان جهازك عمودياً تظهر طبقة **«أدر جهازك للوضع الأفقي»** مع زر
  **«تفعيل الوضع الأفقي تلقائياً»** الذي يطلب ملء الشاشة ثم يقفل الاتجاه أفقياً
  (`screen.orientation.lock('landscape')`).
- **تجميد ذكي**: تتوقف المحاكاة أثناء ظهور طبقة التدوير حتى لا تُقصى وأنت تدير جهازك، وتعود فوراً عند التدوير.
- **إعادة ضبط فورية للمقاسات**: عند كل تدوير يُعاد قياس الكانفس للمحرّكين ثنائي وثلاثي الأبعاد معاً.
- **أزرار جاهزة**: 🔄 داخل المباراة (مع أزرار اللمس)، وفي قائمة الإيقاف المؤقت (`Esc`)، وفي الإعدادات.
- **تحكّم كامل**: الإعدادات ← **«اللعب بالعرض»** (افتراضياً مفعّل لأجهزة اللمس والنوافذ الضيقة) أو زر
  «المتابعة بالوضع العمودي» إن أردت اللعب عمودياً.
- **HUD مضبوط للعرض**: على الشاشات القصيرة (هاتف بالعرض) يتكثّف الشريط العلوي والعصا والأزرار،
  وواجهة المتجر والخزنة تستفيد من العرض بدل التمرير الطويل.
- **تثبيت كتطبيق**: `manifest.webmanifest` يثبّت `orientation: landscape` و`display: fullscreen`
  عند إضافة اللعبة للشاشة الرئيسية.
- على **آيفون** لا يسمح النظام بالقفل البرمجي، فتبقى التعليمات اليدوية (أوقف قفل الاتجاه من مركز التحكم).
...
public/
  ...
  js/orientation.js اللعب بالعرض: كشف الاتجاه، طبقة «أدر جهازك»، قفل landscape، وإعادة ضبط المقاسات
  js/net.js     WebSocket + REST

tests/          اختبارات شاملة (٢٥ منطق + ٣٣ رسم ثلاثي الأبعاد + ٨٢ عميل + ٢١ أونلاين)
...
npm run test:client    # ٨٢ اختبار عميل: DOM حقيقي (jsdom) + ١٤٠٠ إطار رسم على ٥ خرائط + ٢٩ للوضع الأفقي
...
حالة الاختبارات الحالية: **٢٥/٢٥** منطق · **٣٣/٣٣** رسم ٣D · **٨٢/٨٢** عميل · **٢١/٢١** أونلاين — بلا أي خطأ.
...
- **يُلعب بالعرض (Landscape)**: كشف تلقائي للاتجاه + طبقة تدوير + قفل `landscape` عبر Screen Orientation API، مع تجميد المحاكاة أثناء التدوير وإعادة ضبط المقاسات فوراً (آيفون يحتاج تدويراً يدوياً).
